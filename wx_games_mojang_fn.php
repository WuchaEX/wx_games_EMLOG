...
function wx_mojang_check_user() {
    ...
    if ($uid > 0) {
        $db = Database::getInstance();
        $row = $db->once_fetch_array("SELECT uid, nickname, photo AS avatar FROM `" . DB_PREFIX . "user` WHERE uid = $uid LIMIT 1");
        if ($row) { return ['uid' => (int)$row['uid'], 'nickname' => $row['nickname'], 'avatar' => $row['avatar']]; }
    }
    return null;
}
...
function wx_mojang_api_get_inventory($uid) {
    ...
    while ($row = $db->fetch_array($result)) {
        ...
        $items[] = ['inv_id' => (int)$row['id'], 'item_id' => (int)$row['item_id'],
            ...
            'effect_data' => stripslashes($row['effect_data']),
            'quantity' => max(0, (int)$row['quantity'] - (int)$row['used']), 'is_active' => (int)$row['is_active']];
    }
    wx_mojang_ok(['items' => $items]);
}
